<fieldset ng-app="calculate">
	<legend>Selecci&oacute;n de Marcas</legend>
	...
	<div ng-controller="ctrlCalcular">
		<div class="row-fluid">
			...
			<div class="span4" style="padding-top:30px;">
				<a href="#" title="Agregar Marca" ng-click="evtAddBrand($event)" class="btn btn-inverse btn-small evtAddBrand"><i class="icon-plus"></i> Agregar Marca</a>
			</div>
		</div>
		<div class="row-fluid">
			<table class="table evtProductsTable">
				<thead>
					<tr>
						...
						<th style="width:45%; text-align:center">Marca</th>
				...
				<tbody>
					<tr ng-repeat="item in items">
						<td><a class="btn btn-danger btn-small" ng-click="evtRemoveBrand(this, $event)" href="#" data-id="{{item.id}}" title="Eliminar"><i class="icon-trash"></i></a></td>
						<td>{{item.codigo}}</td>
						<td>{{item.marca}}</td>
						<td style="text-align:right">
							<div class="input-append">
  								<input ng-model="item.importe" ng-change="change($index, this, $event)" numericbinding step="0.01" min="0" max="{{getAvailableDiscount()}}" data-id="{{item.id}}" class="evtChange input-small numeric" type="number" />
...
if ($update) {
	$cs->registerScript('solicitudes.jquery.main', "
		...
		app.controller('ctrlCalcular', function(\$scope) {
			...
			\$scope.evtAddBrand = function(\$event) {
				...
				jQuery.ajax({
					url: '".CController::createUrl('solicitudes/GuardarDescuentosCooperacion', array('id'=>$id))."',
					type: 'POST',
			...
			\$scope.evtRemoveBrand = function(el, \$event) {
				...
				var index = el.\$index;
				jQuery.ajax({
					url: '".CController::createUrl('solicitudes/EliminarDescuentoCooperacion', array('id'=>$id))."',
					type: 'POST',
					data: {
	...
	$cs->registerScript('solicitudes.jquery.main.form', "
		$(document).on('blur', '.evtChange', function(e) {
			...
			var el = $(this);
			$.ajax({
				url: '".CController::createUrl('solicitudes/ActualizaImporteMarcasCooperacion', array('id'=>$id))."',
				type: 'POST',
				data: {
					YII_CSRF_TOKEN:'".Yii::app()->request->csrfToken."',
					id: el.attr('data-id'),
